...
The verifier's natural-language assessment uses an in-process local model adapter, not a hosted model or local HTTP inference server. `llama-cpp-python` is the planned runtime behind an injected assessor seam. A user/admin manually provisions a compatible instruction-tuned GGUF model outside the repository and configures its explicit local path plus model identity/checksum before a run; model auto-download, `-hf` shortcuts, network model APIs, and fallback hosted models are prohibited. JSON-schema-constrained output and post-generation receipt validation are both required: schema compliance does not authorize the model to invent citations.
